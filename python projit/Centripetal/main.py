...
def circleToPolygon(circle, quad):
    ...
    if (x+r > w.lowerx) and (x-r < w.upperx) and (y+r > w.lowery) and (y-r < w.uppery):
        ...
        if polygonContainPoint(cpos, quad):
            return (True, cpos, cpos.normalize())

    return  (False, None, None)


def polygonContainPoint(p, quad):
    flags = 0
    i = 0
    j = 4-1
    c = False
...
class CentripetalGame(Widget):
    ...
    def generate_level(self):
        ...
        ring_spacing = width / 60.0 / n_rings *7
...
class CentripetalMenu(Widget):
    def start(self):
        self.t = 0.0
        with self.canvas:
            self.logo = Rectangle(size= (405, 153), pos=(self.size[0]/2 - 405 /2, self.size[1]/2- 153/2 + 150 ),
                                   source = 'logo.png'),

    def update(self, dt):
        self.logo = (self.size[0]/2 - 405/2 , self.size[1]/2 - 153/2 + 157 + sin(self.t *3) *10)
